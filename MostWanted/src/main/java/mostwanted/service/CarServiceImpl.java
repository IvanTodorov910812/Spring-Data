...
public class CarServiceImpl implements CarService {
    ...
    @Autowired
    public CarServiceImpl(CarRepository carRepository, FileUtil fileUtil, RacerRepository racerRepository, Gson gson, ValidationUtil validationUtil, ModelMapper modelMapper) {
    ...
    public String importCars(String carsFileContent) {
        ...
        Arrays.stream(this.gson.fromJson(carsFileContent, CarImportDTO[].class)).forEach(carImportDTO -> {
            ...
            importResult.append(String.format(Constants.SUCCESSFUL_IMPORT_MESSAGE,
                    carEntity.getClass().getSimpleName(), carImportResult))
                    .append(System.lineSeparator());
        });
